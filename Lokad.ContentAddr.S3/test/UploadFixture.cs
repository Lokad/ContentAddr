...
namespace Lokad.ContentAddr.S3.Tests
{
    public abstract class UploadFixture
    {
        ...
        [Fact]
        public async Task medium_file_split_buffered()
        {
            var file = FakeFile(1024 * 1024 * 6);

            WrittenBlob r;
            using (var w = WriteStore.StartWriting())
            {
                for (var i = 0; i < file.Length; i += 1024 * 1024)
                    await w.WriteAsync(file, i, 1024 * 1024, CancellationToken.None);

                r = await w.CommitAsync(CancellationToken.None);
            }

            Assert.Equal("D740F660753A4A38A24D739D768410E9", r.Hash.ToString());
            Assert.Equal(1024 * 1024 * 6, r.Size);

            var a = ReadStore[new Hash("D740F660753A4A38A24D739D768410E9")];

            Assert.True(await a.ExistsAsync(CancellationToken.None));
            Assert.Equal(1024 * 1024 * 6, await a.GetSizeAsync(CancellationToken.None));
